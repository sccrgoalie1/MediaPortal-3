...
namespace Mediaportal.TV.Server.TVLibrary
{
  ...
  /// </summary>
  public class TvController : IInternalControllerService, IDisposable, ITvServerEvent, ICiMenuCallbacks
  {
          ...
          if (_cards != null)
          {
            if (_cards.ContainsKey(cardId))
            {
              ...
              if (devicePath.Length > 0)
              {
                ...
                cardPresent = (from t in _localCardCollection.Cards where t.DevicePath == devicePath select t.CardPresent).FirstOrDefault();
              }
            }
          }
        }
